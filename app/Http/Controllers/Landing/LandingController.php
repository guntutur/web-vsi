...
class LandingController extends Controller
{
    public function getTingkatAktivitas()
    {
        ...
        if ($pressRelease) {
            $pressRelease->link = config('app.url') . "/press-release/" . $pressRelease->route;
        ...
        if ($tanggapanKejadian) {
            $tanggapanKejadian->link = config('app.url') . "/tanggapan-kejadian/" . $tanggapanKejadian->route;
        ...
        if ($kajianKejadian) {
            $kajianKejadian->link = config('app.url') . "/kajian-kejadian/" . $kajianKejadian->route;
    ...
    public function index()
    {
        ...
        if (count($pressRelease) > 0) {
            foreach ($pressRelease as $row) {
                ...
                $row->link = config('app.url') . "/press-release/" . $row->route;
                array_push($news, $row);
            }
        ...
        if (count($tanggapanKejadian) > 0) {
            foreach ($tanggapanKejadian as $row) {
                ...
                $row->link = config('app.url') . "/tanggapan-kejadian/" . $row->route;
                $row->thumbnail = config('app.url') . "/storage/" . $row->thumbnail;
                array_push($news, $row);
            }
        ...
        if (count($kajianKejadian) > 0) {
            foreach ($kajianKejadian as $row) {
                ...
                $row->link = config('app.url') . "/kajian-kejadian/" . $row->route;
                $row->thumbnail = config('app.url') . "/storage/" . $row->thumbnail;
                array_push($news, $row);
            }
